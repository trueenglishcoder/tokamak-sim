...
def _write_v7_no_step_export(
    ...
    *,
    ...
    """Write a minimal v7 no-step learned-controller bundle and return derivative limits."""
    n_pfc = int(model.pfc.n_coils)
    n_sol = int(model.sol.n_coils)
    action_dim = n_pfc + n_sol
    slices, obs_dim = _feature_slices(
        ...
        n_angles=n_angles,
        preview_steps=target_preview_steps,
        feature_order=CONTROLLER_STATE_V7_NO_STEP_NORM_FEATURE_ORDER,
    )
    derivative_scale = np.linspace(1.0e6, 9.0e6, action_dim, dtype=float)
    export.mkdir()
    schema = {
        "observation_kind": "controller_state_v7_no_step_norm",
        "obs_dim": obs_dim,
        "action_dim": action_dim,
        "n_active_total": action_dim,
        "n_pfc": n_pfc,
...
def test_learned_controller_v7_no_step_accepts_export_without_episode_phase(tmp_path: Path) -> None:
    """The v7 no-step schema should load without step_norm and keep the action contract."""
    cfg = _load_config_with_initial()
    model = _model_from_config(cfg)
    export = tmp_path / "export_v7"
    derivative_scale = _write_v7_no_step_export(export, model=model, n_angles=8, mean_bias=0.25)
    controller = make_controller("learned_magnetic_controller", config={"export_dir": export})
    call = build_controller_runtime_call("learned_magnetic_controller", _runtime_context(model, cfg, n_angles=8))

    assert controller.schema["observation_kind"] == "controller_state_v7_no_step_norm"
    assert "step_norm" not in controller.schema["feature_order"]
    assert controller.schema["feature_order"] == [name for name in CONTROLLER_STATE_V6_FEATURE_ORDER if name != "step_norm"]
...
def test_learned_controller_v7_no_step_integral_features_match_training_units(tmp_path: Path) -> None:
    """v7 no-step exports should keep v6 rate, integral, previous-action, and preview features."""
    cfg = _load_config_with_initial()
    model = _model_from_config(cfg)
    export = tmp_path / "export_v7"
    _write_v7_no_step_export(export, model=model, n_angles=6, mean_bias=0.0)
    controller = make_controller("learned_magnetic_controller", config={"export_dir": export})
    ctx = _runtime_context(model, cfg, n_angles=6)
    ...
    assert "step_norm" not in slices
    for required in (
        "ip_ref_rate",
        "boundary_ref_rate",
        "ip_measured_rate",
