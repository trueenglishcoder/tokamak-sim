...
from tokamak_control.control.learned_magnetic_controller import CONTROLLER_STATE_V3_FEATURE_ORDER
...
def _feature_slices(*, action_dim: int, n_angles: int, preview_steps: int) -> tuple[dict[str, list[int]], int]:
    """Return v3 feature slices and total observation size."""
    sizes = {
        "step_norm": 1,
        "ip": 1,
        "ip_ref": 1,
    ...
    cursor = 0
    for name in CONTROLLER_STATE_V3_FEATURE_ORDER:
        size = int(sizes[name])
...
def _write_v3_export(
    export: Path,
    *,
    model: PlasmaModel,
    n_angles: int,
    mean_bias: float = 0.0,
    action_contract: str = "delta_jdot_derivative_command_v3",
) -> tuple[np.ndarray, np.ndarray]:
    """Write a minimal v3 learned-controller bundle and return derivative/delta limits."""
    ...
    return derivative_scale, delta_limits
...
def test_learned_controller_v3_outputs_absolute_next_currents(tmp_path: Path) -> None:
    """The v3 controller accumulates delta-Jdot and returns absolute next currents."""
    ...
    derivative_scale, delta_limits = _write_v3_export(export, model=model, n_angles=8, mean_bias=0.5)
    ...
    requested_delta = float(np.tanh(0.5))
    assert np.allclose(first_jdot, requested_delta * delta_limits, rtol=1.0e-6, atol=1.0e-6)
    assert np.allclose(controller._previous_requested_delta_action_norm, requested_delta)
    assert np.allclose(controller._previous_derivative_command_norm, first_jdot / derivative_scale)
    ...
    assert np.allclose(second_jdot, 2.0 * requested_delta * delta_limits, rtol=1.0e-6, atol=1.0e-6)
...
def test_learned_controller_zero_action_holds_currents(tmp_path: Path) -> None:
    ...
    model = PlasmaModel.from_settings(grid=cfg.grid, pfc=cfg.pfc, sol=cfg.sol, settings=cfg.physics)
    export = tmp_path / "export"
    _write_v3_export(export, model=model, n_angles=4, mean_bias=0.0)
    controller = make_controller("learned_magnetic_controller", config={"export_dir": export})
...
def test_learned_controller_rejects_old_action_contract(tmp_path: Path) -> None:
    """Old learned exports must not silently run under the v3 plant contract."""
    cfg = load_config(CONFIG, initial_currents_path=INITIAL)
    model = PlasmaModel.from_settings(grid=cfg.grid, pfc=cfg.pfc, sol=cfg.sol, settings=cfg.physics)
    export = tmp_path / "export_old"
    _write_v3_export(export, model=model, n_angles=4, action_contract="delta_jdot_derivative_command_v2")

    with pytest.raises(ValueError, match="delta_jdot_derivative_command_v3"):
